...
<?
	...
	for ($i=0; $i < $count; $i++) { 
		...
		if (!$upfile_error[$i]) {
			...
			$new_file_name = $new_file_name_."_".$i;
			$copied_file_name[$i] = $new_file_name_.".".$file_ext;
			$uploaded_file[$i] = $upload_dir.$copied_file_name[$i];

			if ($upfile_size[$i] > 10000000) {
				echo "<script>
						alert('업로드 파일 크기가 지정된 용량 (10mb)를 초과합니다.<br>);
						history.go(-1);
					  </script>";
					  exit;
			}
