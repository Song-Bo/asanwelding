<?
	$url = "http://$_SERVER[HTTP_ROOT]/asan";
?>
<div class="nav_wrap">
	<div class="nav">
		커뮤니티
	</div>
	<ul class="sub_nav">
				<li class="sub_nav1"><a href="$url/community/free/list.php" class="sub_nav_txt">자유 게시판</a></li>
				<li class="sub_nav2"><a href="$url/community/job/list.php" class="sub_nav_txt">취업소식</a></li>
				<li class="sub_nav3"><a href="$url/community/download/list.php" class="sub_nav_txt">자료실</a></li>
				<li class="sub_nav4"><a href="$url/community/qna/list.php" class="sub_nav_txt">Q & A</a></li>
				<li class="sub_nav5"><a href="$url/admin/notice/list.php" class="sub_nav_txt">공지사항</a></li>
			</ul>
</div><!-- end of nav_wrap -->